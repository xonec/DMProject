...
class DMPic:
    ...
    def GetCapture( stax, stay, endx, endy):
        ...
        return np.array(im)
    @staticmethod
    def FindPic( x1, y1, x2, y2, path, thd=0.9, type=1):
        '''
        找图
        :param x1: 起点X
        :param y1: 起点Y
    ...
    @staticmethod
    def CmpColor(x, y, color, sim: float):
        '''
        比色
        :param x: X坐标
        :param y: Y坐标
        ...
        :return: 真或假
        '''
        img = GetCapture(x - 1, y - 1, x + 1, y + 1)
        img = img[1][1]
    ...
    @staticmethod
    def FindColor(x1, y1, x2, y2, des, sim: float):
        '''
        找色
        :param x1: 起点X
        :param y1: 起点Y
        ...
        '''
        img = GetCapture(x1, y1, x2, y2)
        res = np.absolute(img - Hex_to_Rgb(des))
        sim = int((1 - sim) * 255)
        res = np.argwhere(np.all(res <= sim, axis=2))
        res = res + (y1, x1)
        return res[:, [1, 0]]
    @staticmethod
    def GetColorNum(x1, y1, x2, y2, des, sim: float):
        '''
        获取颜色数量
        :param x1: 起点X
        :param y1: 起点Y
        ...
        :return:
        '''
        return len(FindColor(x1, y1, x2, y2, des, sim))
    @staticmethod
    def FindMultColor(stax, stay, endx, endy, des):
        '''
        多点找色
        :param stax:
        :param stay:
        ...
        return np.array([-1, -1])
    @staticmethod
    def FindPicEx(x1, y1, x2, y2, path, thd=0.9, MIN_MATCH_COUNT=8):
        '''
        全分辨率找图
        :param x1:
        :param y1:
    ...
    def _FilterRec(res, loc):
        ...
        return np.array(pts)
    @staticmethod
    def FindMultPic(x1, y1, x2, y2, path, thd=0.8):
        '''
        多目标找图
        :param x1:
        :param y1:
        ...
        if len(loc):
            resc = _FilterRec(res, loc)
            return resc + (h / 2 + x1, w / 2 + y1)
        else:
            return [[-1, -1]]
    @staticmethod
    def FindPic_TM(x1, y1, x2, y2, path, thd=0.9):
        '''
        找透明图，透明色为黑色
        :param x1: 起点X
        :param y1: 起点Y
        ...
        if 1 - minVal >= thd:
            ...
            return -1, -1
    @staticmethod
    def GetCaptre_TM(x1, y1, x2, y2, path, times=5):
        '''
        ...
        return a
    @staticmethod
    def StressShow(stax, stay, endx, endy, des, type=0):
        '''
    ...
    def SetDict(path):
        ...
        with open(path, 'r', encoding='GBK') as f:
            ...
            for line in lines:
                parts = line.split("$")
                # des.setdefault(parts[1],parts[0])
                bin_str = ''
                for c in parts[0]:
                    byte = int(c, 16)
        ...
        return des
    @staticmethod
    def FindString(x1, y1, x2, y2, strs, color, thd, DIict):
        if strs not in DIict:
            print('字符串不存在')
            return -1, -1
        else:
            arr = DIict[strs]
        img = StressShow(x1, y1, x2, y2, color, 1)
    ...
    @staticmethod
    def Ocr(x1, y1, x2, y2, des, thd, DIict):
        dess = des.split('|')
        des = [i[0:6] for i in dess]
        des = [np.array(Hex_to_Rgb(d)) for d in des]
        pds = [i[-6:] for i in dess]
